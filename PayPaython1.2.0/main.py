...
class PayPay:
    def __init__(self,phone:str,password:str,client_uuid:str=str(uuid.uuid4()),proxy:dict=None):
        ...
        try:
            ...
            if login.json()["response_type"]=="ErrorResponse":
                ...
                self.refid=login["otp_reference_id"]
    ...
    def login(self,otp:str):
        ...
        login.json()["client_uuid"] = str(self.uuuid)
        return login.json()
    ...
    def receive(self,pcode:str,password:int=4602) -> dict:
        info=self.session.get(f"https://www.paypay.ne.jp/app/v2/p2p-api/getP2PLinkInfo?verificationCode={pcode}",headers=self.headers,proxies=self.proxy)
        recevej = {
